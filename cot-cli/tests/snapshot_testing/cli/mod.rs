...
fn manpages() {
    ...
    insta::with_settings!({ filters => [GENERIC_FILTERS, TEMP_PATH_FILTERS].concat().to_owned() },{ assert_cmd_snapshot!(cot_cli!("cli", "manpages", "-o", tempdir.path())) });
}

#[test]
fn completions_missing_shell() {
    insta::with_settings!({ filters => [GENERIC_FILTERS, TEMP_PATH_FILTERS].concat().to_owned() },{ assert_cmd_snapshot!(cot_cli!("cli", "completions")) });
}
