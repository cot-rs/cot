...
/// Computes the anchor fragment that cot-site's Markdown renderer (comrak's
/// `Anchorizer`, using the GFM slug algorithm) generates for a `` `[path]` ``
/// heading: lowercase, then drop everything but letters/digits/underscores
/// (spaces become hyphens). Every path segment here is a lowercase Rust
/// identifier (letters, digits, underscores only, no spaces), so the only
/// characters the algorithm actually strips are the heading's own brackets
/// and the dots joining the segments - i.e. it reduces to concatenating the
/// segments as-is.
