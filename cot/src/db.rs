...
use query::Query;
use query::expr::SqlQueryBuilder;
...
impl Transaction<'_> {
    ...
    ///
    /// This is the transaction-scoped counterpart of [`Database::raw`].
    ///
    /// # Errors
    ///
    ...
    /// transaction
    ///     .raw("CREATE TABLE test (id INTEGER PRIMARY KEY AUTOINCREMENT, name TEXT)")
    ///     .await?;
    /// transaction.commit().await?;
    /// # Ok(())
    /// # }
    /// ```
    pub async fn raw(&mut self, query: &str) -> Result<StatementResult> {
        self.raw_with(query, &[]).await
    }

    /// Executes a raw SQL query with parameters within the transaction.
    ///
    /// This is the transaction-scoped counterpart of [`Database::raw_with`].
    ///
    /// # Errors
    ///
    /// This method can return an error if the query is invalid.
    ...
    ///     .await?;
    /// transaction
    ///     .raw_with("INSERT INTO test (name) VALUES (?)", &[&"cot"])
    ///     .await?;
    /// transaction.commit().await?;
    /// # Ok(())
    /// # }
    ...
    pub async fn raw_with(
        ...
        let result = match &mut self.inner {
            #[cfg(feature = "sqlite")]
            TransactionImpl::Sqlite(inner) => inner.raw_with(query, values).await?,
            #[cfg(feature = "postgres")]
            TransactionImpl::Postgres(inner) => inner.raw_with(query, values).await?,
            #[cfg(feature = "mysql")]
            TransactionImpl::MySql(inner) => inner.raw_with(query, values).await?,
        };

        Ok(result)
    }

    /// Executes a raw SQL query within the transaction and maps the results to
    /// a model type.
    ///
    /// This is the transaction-scoped counterpart of [`Database::raw_as`].
    ///
    /// # Safety
    ///
    ...
    /// let mut transaction = db.begin().await?;
    /// transaction
    ///     .raw("CREATE TABLE user (id INTEGER PRIMARY KEY AUTOINCREMENT, username TEXT)")
    ///     .await?;
    /// transaction
    ///     .raw("INSERT INTO user (username) VALUES ('jondoe')")
    ///     .await?;
    ///
    /// let users = transaction.raw_as::<User>("SELECT * FROM user").await?;
    /// assert_eq!(users.len(), 1);
    /// transaction.commit().await?;
    /// # Ok(())
    /// # }
    /// ```
    pub async fn raw_as<T: Model>(&mut self, query: &str) -> Result<Vec<T>> {
        self.raw_as_with(query, &[]).await
    }

    /// Executes a raw SQL query with parameters within the transaction and maps
    /// the results to a model type.
    ///
    /// This is the transaction-scoped counterpart of [`Database::raw_as_with`].
    ///
    /// # Safety
    ///
    /// This method executes the raw SQL string without any sanitization.
...
impl LikeExprBuilder for Transaction<'_> {
    fn like_expr(
        &self,
        lhs: SimpleExpr,
        glob_pattern: &str,
        ...
        match &self.inner {
            #[cfg(feature = "sqlite")]
            TransactionImpl::Sqlite(_) => {
                impl_sqlite::build_like_expr(lhs, glob_pattern, case_sensitivity)
            }
            #[cfg(feature = "postgres")]
            TransactionImpl::Postgres(_) => {
                impl_postgres::build_like_expr(lhs, glob_pattern, case_sensitivity)
            }
            #[cfg(feature = "mysql")]
            TransactionImpl::MySql(_) => {
                impl_mysql::build_like_expr(lhs, glob_pattern, case_sensitivity)
            }
        }
    }
}
...
trait RawExecutor {
    ...
    /// Returns the SQL query builder used to render backend-specific
    /// expressions (e.g. pattern matching) for this executor's dialect.
    fn as_sql_query_builder(&self) -> &dyn SqlQueryBuilder;

    /// Rejects models that aren't allowed in this executor's context (e.g.
    /// application models used in migrations).
...
impl RawExecutor for &Database {
    ...
    fn ensure_model_allowed<T: Model>(&self) -> Result<()> {
        Database::ensure_model_allowed::<T>(self)
    }
...
impl RawExecutor for Transaction<'_> {
    ...
    fn ensure_model_allowed<T: Model>(&self) -> Result<()> {
        check_model_allowed::<T>(self.context)
    }
...
/// Rejects models that don't match the given database context: application
/// models cannot be used in migrations, and migration models cannot be used
/// outside of them. Shared between [`Database`] and [`Transaction`].
fn check_model_allowed<T: Model>(context: DatabaseContext) -> Result<()> {
    match (context, T::MODEL_TYPE) {
...
impl Database {
    ...
    async fn insert_or_update_generic<T: Model, E: RawExecutor>(
        ...
        executor.ensure_model_allowed::<T>()?;
    ...
    async fn query_generic<T: Model, E: RawExecutor>(
        ...
        query.add_order_by_to_statement(&mut select);
    ...
    async fn delete_generic<T: Model, E: RawExecutor>(
        ...
        query.add_filter_to_statement(&mut delete, executor.as_sql_query_builder())?;
    ...
    pub async fn raw_as_with<T: Model>(
        ...
        let mut executor: &Database = self;
        let rows = executor
            .fetch_all(&RawStatement { sql: query, values })
            .await?;
        rows.into_iter().map(T::from_db).collect::<Result<_>>()
    }
