...
impl MigrationEngine {
    ...
    fn rollback_plan<'a>(
        ...
        // other apps only when they depend on that seed set.
...
impl Operation {
    ...
    /// Returns a builder for an operation that removes a field from a model.
    ///
    /// Typically, you shouldn't need to use this directly. Instead, in most
    /// cases, this can be automatically generated by the Cot CLI.
    ///
    ...
    /// Returns a builder for an operation that removes a model.
    ///
    /// Typically, you shouldn't need to use this directly. Instead, in most
    /// cases, this can be automatically generated by the Cot CLI.
    ///
    ...
    ///
    /// // Then remove it
    /// const OPERATION: Operation = Operation::remove_model()
    ///     .table_name(Identifier::new("todoapp__my_model"))
    ///     .fields(FIELDS)
    ///     .build();
    ///
    ...
    /// Returns a builder for an operation that alters an existing field on a
    /// model.
    ///
    /// Typically, you shouldn't need to use this directly. Instead, in most
    /// cases, this can be automatically generated by the Cot CLI.
    ...
    /// # Examples
    ///
    /// Note: `AlterField` is currently only supported on PostgreSQL and
    /// MySQL backends; SQLite's `ALTER TABLE` does not support modifying
    /// existing columns.
    ///
    /// ```no_run
    /// use cot::db::migrations::{Field, Operation};
    ...
    ///     .fields(FIELDS)
    ///     .build();
    /// let database = cot::db::Database::new("postgres://localhost/example").await?;
    /// CREATE_OPERATION.forwards(&database).await?;
    ///
    /// // Then alter the `name` field to allow null values
    /// const OPERATION: Operation = Operation::alter_field()
    ///     .table_name(Identifier::new("todoapp__my_model"))
    ///     .old_field(Field::new(
    ///         Identifier::new("name"),
    ///         <String as DatabaseField>::TYPE,
...
enum OperationInner {
    ...
    /// Remove a model with the given fields
    RemoveModel {
        table_name: Identifier,
        fields: &'static [Field],
    },
...
impl RemoveModelBuilder {
    ...
    /// Sets the name of the table to remove.
    ///
    /// # Cot CLI Usage
    ///
    /// Typically, you shouldn't need to use this directly. Instead, in most
    ...
    /// Sets the fields to store for potential backwards migration.
    ///
    /// # Cot CLI Usage
    ///
    /// Typically, you shouldn't need to use this directly. Instead, in most
...
/// A builder for altering an existing field on a model.
///
/// Typically, you shouldn't need to use this directly. Instead, in most
/// cases, this can be automatically generated by the Cot CLI.
///
/// Note: `AlterField` is currently only supported on PostgreSQL and MySQL
/// backends; SQLite's `ALTER TABLE` does not support modifying existing
/// columns.
///
/// # Examples
///
...
impl Default for AlterFieldBuilder {
    fn default() -> Self {
        Self::new()
    }
}
...
impl AlterFieldBuilder {
    ...
    /// Sets the previous definition of the field that is being altered.
    ///
    /// This is used to roll the migration back via [`Operation::backwards`].
    ///
    /// # Cot CLI Usage
    ///
    /// Typically, you shouldn't need to use this directly. Instead, in most
    ...
    /// Sets the new definition of the field that is being altered.
    ///
    /// # Cot CLI Usage
    ///
    /// Typically, you shouldn't need to use this directly. Instead, in most
    ...
    /// # Panics
    ///
    /// Panics if [`Self::table_name`], [`Self::old_field`] or
    /// [`Self::new_field`] have not been called.
    #[must_use]
    pub const fn build(self) -> Operation {
...
mod tests {
    ...
    fn test_operation_alter_field() {
        ...
        {
            assert_eq!(table_name.to_string(), "testapp__test_model");
            assert_eq!(old_field.name.to_string(), "name");
            assert_eq!(old_field.ty, ColumnType::Text);
            assert_eq!(new_field.name.to_string(), "name");
            assert!(new_field.null);
        } else {
            panic!("Expected OperationInner::AlterField");
        }
    }

    // Note: AlterField is only tested against PostgreSQL and MySQL because
    // SQLite's `sea_query` backend does not support `ALTER TABLE ... MODIFY
    // COLUMN` (it panics). These tests follow the pattern used by the
    // ignored `*_postgres` / `*_mysql` tests generated by
    // `cot_macros::dbtest`.

    async fn run_alter_field_forwards(test_db: &mut TestDatabase) {
        const FIELDS: &[Field] = &[
            Field::new(Identifier::new("id"), <i32 as DatabaseField>::TYPE)
                .primary_key()
                .auto(),
        ...
        let alter_operation = Operation::alter_field()
            .table_name(Identifier::new("testapp__test_model"))
            .old_field(Field::new(
                Identifier::new("name"),
                <String as DatabaseField>::TYPE,
    ...
    async fn run_alter_field_backwards(test_db: &mut TestDatabase) {
        const FIELDS: &[Field] = &[
            Field::new(Identifier::new("id"), <i32 as DatabaseField>::TYPE)
                .primary_key()
                .auto(),
    ...
    #[cot::test]
    async fn test_alter_field_operation_forwards_postgres() {
        let mut database =
            TestDatabase::new_postgres(stringify!(test_alter_field_operation_forwards))
                .await
                .expect("failed to create PostgreSQL test database");
        run_alter_field_forwards(&mut database).await;
        database
            .cleanup()
            .await
            .expect("failed to clean up PostgreSQL test database");
    }

    #[ignore = "Tests that use MySQL are ignored by default"]
    #[cot::test]
    async fn test_alter_field_operation_forwards_mysql() {
        let mut database = TestDatabase::new_mysql(stringify!(test_alter_field_operation_forwards))
            .await
    ...
    #[ignore = "Tests that use PostgreSQL are ignored by default"]
    #[cot::test]
    async fn test_alter_field_operation_backwards_postgres() {
        let mut database =
            TestDatabase::new_postgres(stringify!(test_alter_field_operation_backwards))
                .await
                .expect("failed to create PostgreSQL test database");
        run_alter_field_backwards(&mut database).await;
        database
            .cleanup()
            .await
            .expect("failed to clean up PostgreSQL test database");
    ...
    #[ignore = "Tests that use MySQL are ignored by default"]
    #[cot::test]
    async fn test_alter_field_operation_backwards_mysql() {
        let mut database =
            TestDatabase::new_mysql(stringify!(test_alter_field_operation_backwards))
                .await
                .expect("failed to create MySQL test database");
        run_alter_field_backwards(&mut database).await;
        database
