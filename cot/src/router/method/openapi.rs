...
use crate::openapi::{
    AsOpenapiOperation, AsPathItem, BoxApiRequestHandler, into_box_api_request_handler,
};
...
impl ApiMethodRouter {
    ...
    /// does not require the handler to implement [`AsOpenapiOperation`].
    ///
...
impl AsPathItem for ApiMethodRouter {
    fn as_path_item(&self, route_context: &RouteContext<'_>) -> aide::openapi::PathItem {
        macro_rules! add_method {
            ($path_item:ident, $method:ident) => {
                if let Some(handler) = &self.inner.$method {
                    $path_item.$method = handler.as_operation(route_context);
                }
            };
        }
        ...
        add_method!(path_item, get);
        add_method!(path_item, head);
        add_method!(path_item, delete);
        add_method!(path_item, options);
        add_method!(path_item, patch);
        add_method!(path_item, post);
        add_method!(path_item, put);
        add_method!(path_item, trace);
...
impl InnerApiHandler {
    ...
    where
        HandlerParams: 'static,
        ApiParams: 'static,
        H: RequestHandler<HandlerParams> + AsOpenapiOperation<ApiParams> + Send + Sync + 'static,
    {
...
impl AsOpenapiOperation for InnerApiHandler {
...
mod tests {
    ...
    #[cot::test]
    async fn method_api_router_fallback() {
    ...
    #[cot::test]
    async fn method_api_router_get() {
    ...
    #[cot::test]
    async fn method_api_router_default_head() {
        ...
        let mut router = api_get(test_handler);
    ...
    #[cot::test]
    async fn method_api_router_multiple() {
        let router = ApiMethodRouter::new()
            .get(test_handler)
            .head(test_handler)
            .delete(test_handler)
    ...
    fn openapi_empty() {
        let router = ApiMethodRouter::new();
        let route_context = RouteContext { param_names: &[] };

        let endpoint = router.as_path_item(&route_context);
    ...
    #[test]
    fn openapi_multiple() {
        let router = api_post(test_handler_with_params).delete(test_handler_with_params);
        let route_context = RouteContext {
            param_names: &["123"],
        };
        ...
        assert!(endpoint.delete.is_some());
        let delete = endpoint.delete.unwrap();
        assert_eq!(delete.parameters.len(), 1);
        assert!(delete.request_body.is_some());
