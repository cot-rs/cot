...
This is a complete example with every key set explicitly to its default value. Fields without a well-defined default (like `secret_key`) are shown as `"..."` and must be set explicitly:
